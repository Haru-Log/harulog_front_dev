...
export const mergeCategory = (
  ...
  receieved_jandi.forEach((item) => {
    if (
      currentGroup.date.toISOString().split('T')[0] ===
      ...
      currentGroup = {
        date: item.date,
        category: { [item.categoryName]: item.activityTime },
      };
    } else {
      // 같은 날짜의 그룹에 추가
  ...
  mergedJandi.forEach((item) => {
    categoryMax.공부 = Math.max(
      categoryMax.공부,
      item.category.공부 ? item.category.공부 : 0
    );
    categoryMax.운동 = Math.max(
      categoryMax.운동,
      item.category.운동 ? item.category.운동 : 0
    );
    categoryMax.독서 = Math.max(
      categoryMax.독서,
      item.category.독서 ? item.category.독서 : 0
    );
  });
...
export const mergeJandi = (initialState: Jandi[], receievedData: Jandi[]) => {
  ...
  let mergedJandi = combinedJandi.filter((x, idx) => {
    if (
      Object.values(x.category).length === 0 &&
      idx < combinedJandi.length - 1 &&
      x.date.toISOString().split('T')[0] ===
        combinedJandi[idx + 1].date.toISOString().split('T')[0]
    ) {
