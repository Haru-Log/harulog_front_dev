...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const getUserProfile = async () => {
      ...
      const myFeed = response.data.filter(
        (x: FeedItem) => x.nickname === nickname
      );
      setFeed(myFeed);
    ...
    <div className="w-full flex justify-center py-16 font-ibm">
      <div className="w-[80%] h-full flex flex-col">
          ...
          <section className="flex w-full items-start">
            <div className="max-w-40 max-h-40 justify-center h-fit mr-5">
              <img
                ...
                className="w-40 h-40 rounded-full object-fill mr-40 whitespace-nowrap"
                alt="Profile"
              />
            </div>
            <div className="flex flex-col w-full">
              <div className="flex w-full items-baseline justify-between">
                ...
                {nickname === myName ? (
                  ...
                  <Button
                    className="bg-point hover:bg-point-hover active:bg-point-active shadow-xl rounded-xl font-bold"
                    onClick={() => setShowConfirmation(true)}
                  >
                    {followState ? (
                      <>
                        <XCircle color="#ffffff" className="mr-2 h-5 w-5" />
                        <p>팔로우 취소</p>
                      </>
                    ) : (
                      <>
                        <UserPlus color="#ffffff" className="mr-2 h-5 w-5" />
                        <p>팔로우</p>
                      </>
                    )}
                  </Button>
              ...
              <div className="flex p-6 justify-between">
                ...
                <ProfileNumber
                  title={'팔로워'}
                  count={userProfile && userProfile.followerCount}
                />
                <ProfileNumber
                  title={'팔로잉'}
                  count={userProfile && userProfile.followingCount}
                />
              </div>
            </div>
          </section>
          <div className="px-5 mb-5">{userProfile?.introduction}</div>
        </div>
        <section>
        ...
        <section className="w-full h-full">
          <div className="w-full flex justify-between">
            <div
              className={`w-full h-fit flex justify-center p-2 cursor-pointer items-center ${
                !feedToggle
                  ...
                  : 'border-t border-t-gray-400 text-gray-400'
              }`}
              onClick={() => setFeedToggle(false)}
            >
              <Archive className="mr-2" strokeWidth={2.5} size={32} />
              피드
            </div>
            <div
              className={`w-full h-fit flex justify-center p-2 cursor-pointer items-center  ${
            ...
            >
              <Mountain className="mr-2" strokeWidth={2.5} size={32} />
              챌린지
            </div>
          </div>
      ...
      {showConfirmation && (
        <ConfirmationModal
          ...
          onConfirm={() => {
            followState ? handleCancelFollow() : handleActionFollow();
