...
const SocialPage = () => {
  const { content, searchToggle, currentPage, setSearchToggle, setUserList, setCurrentPage, setTotalPage } = useContentStore();
    ...
    <div className="w-full flex justify-center mt-10 font-ibm">
      <div className="w-full h-full flex flex-col mx-10 items-center">
        <div className="w-full flex justify-between">
          ...
          <div
            className={`w-full h-fit flex justify-center p-2 cursor-pointer items-center  ${searchToggle === 'followings' ? 'border-t-4 border-black font-bold' : 'border-t-2 border-t-gray-400 text-gray-400'}`}
            onClick={() => setSearchToggle('followings')}
          >
            팔로잉
          </div>
          <div
            className={`w-full h-fit flex justify-center p-2 cursor-pointer items-center  ${searchToggle === 'followers' ? 'border-t-4 border-black font-bold' : 'border-t-2 border-t-gray-400 text-gray-400'}`}
