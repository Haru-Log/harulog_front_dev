...
const RecordPage = () => {
  ...
  const [hour, setHour] = useState(
    id
      ? feed.categoryName === '기상'
        ? Math.floor(feed.activityTime / 60)
        : 0
      : 0
  );
  ...
  const categoryRef: RefObject<HTMLButtonElement> =
  ...
  const contentRef: RefObject<HTMLTextAreaElement> =
    ...
    <div className="flex flex-col mx-10 mt-10 border-2 rounded-xl px-10 py-10 font-ibm">
      <div className="flex flex-row">
        <div className="flex flex-col mr-10">
          ...
          <div className="flex flex-row">
            <span className="font-bold mr-5 mt-5 w-16">날짜</span>
            <Calendar
              mode="single"
              selected={date}
              onSelect={(e) => {
      ...
      <div className="flex justify-between mt-10">
        <Button
          ...
          onClick={() => navigate(`/feed/${id}`, { replace: true })}
        >
          취소
        </Button>
        {id ? (
          <Button
            className="bg-red-500 rounded-lg text-sm w-28 py-2 hover:ring-2 hover:ring-point hover:bg-red-800 active:bg-point-active drop-shadow-md"
        ...
        <Button
          className="bg-point rounded-lg text-sm w-28 py-2 hover:ring-2 hover:ring-point hover:bg-point-hover active:bg-point-active drop-shadow-md"
