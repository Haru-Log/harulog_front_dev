  ...
  useEffect(() => {
    ...
    const fetchFeedDetails = async () => {
      try {
        ...
        setFeed({
          ...feedDetails.data,
          createdAt: new Date(feedDetails.data.createdAt),
          updateAt: new Date(feedDetails.data.updateAt),
          imgUrl: feedImg,
        });
        setLiked(feedDetails.data.likedByUser);

        const profileImg = await fetchImgFromFirebase(
          feedDetails.data.profileImg
        );
      ...
      <div className="w-[50%] h-full p-12 flex flex-col">
        <section className="w-full flex flex-row items-center h-fit justify-between mb-5">
          <div className="flex flex-row items-center h-fit">
            <img
              src={profileImg}
              alt="프로필 이미지"
              className="w-20 h-20 rounded-full mr-10 cursor-pointer"
              onClick={() => navigate(`/profile/${feed.nickname}`)}
            />
            <div
              className="items-center text-3xl whitespace-nowrap font-bold cursor-pointer"
              onClick={() => navigate(`/profile/${feed.nickname}`)}
            >
              {feed?.nickname}
            </div>
          ...
          {localStorage.getItem('nickname') === feed.nickname ? (
            <div className="flex items-center justify-center">
              <div
                ...
                onClick={() => navigate(`/feed/edit/${post_id}`)}
              >
                <Pencil size={30} />
              </div>
              <Button
        ...
        <section className="flex items-center justify-between">
          <div className="flex flex-row">
            <div
              className={`text-white px-3 py-1 rounded-full h-fit w-fit text-center mr-3 text-2xl bg-${feed?.categoryName} whitespace-nowrap`}
            >
              {feed?.categoryName}
            </div>
            <div
              className={`text-white px-3 py-1 rounded-full h-fit w-fit text-center text-2xl bg-${feed?.categoryName} whitespace-nowrap`}
            >
              {feed?.categoryName === '기상'
                ? `${Math.floor(feed.activityTime / 60)}시 ${
                    feed.activityTime % 60
                  }분`
                : `${feed?.activityTime}분`}
            </div>
          </div>
        ...
        <section className="mt-5 flex items-center h-fit">
          ...
          <div className="ml-5 text-4xl font-bold">{feed?.likeCount}</div>
        </section>
        <section className="mt-5 text-4xl">{feed?.content}</section>
        <form onSubmit={submitComment} className="w-full relative flex mt-5">
          ...
          />
          <Button type="submit" className="absolute right-1 text-[#92C7CF]">
            전송
          </Button>
        </form>
