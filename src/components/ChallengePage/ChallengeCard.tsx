...
const ChallengeCard: React.FC<ChallengeItem> = ({ challenge_id, category_id, category_name, chatroom_id, challenge_title, challenge_content, challenge_goal, challenge_image, participants, submission, start_date, end_date, created_at, updated_at }) => {
  ...
  return (
    <div className="cursor-pointer transform transition-transform hover:scale-110 drop-shadow-xl" onClick={() => navigate(`/challenge/${challenge_id}`)}>
      <div className="w-[90%] h-[90%] aspect-square rounded-xl">
        ...
        <div className={`absolute top-0 right-[10%] text-white text-xs font-bold px-3 py-1 mr-2 mt-2 rounded-lg bg-${category_name}`}>
        {category_name}
      </div>
      <div className="absolute bottom-[10%] left-0 right-0 p-3 pt-9 bg-gradient-to-b from-transparent to-slate-800 rounded-b-xl w-[90%]">
        <span className="text-white text-lg font-semibold whitespace-nowrap">{challenge_title}</span>
