...
const FeedCard: React.FC<FeedCardType> = ({
  ...
  return (
    <div
      className="w-96 flex flex-col items-start cursor-pointer transform transition-transform hover:scale-110"
    ...
    >
      <div className="mb-3">
        <img
          src={postImg}
          alt="피드 이미지"
          className="rounded-xl h-fit w-96 transform transition-transform hover:scale-110"
          ref={imgRef}
        />
      </div>
      <div className="flex flex-row w-full justify-start text-xs h-6">
        <div
          className={`text-white px-3 py-1 rounded-full h-fit w-fit text-center mr-5 bg-${categoryName}`}
        >
          {categoryName}
        </div>
