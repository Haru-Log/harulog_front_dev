  ...
  return (
    <TableRow className='text-2xl text-center whitespace-nowrap text-white bg-[#92C7CF88] border-none hover:bg-[#92C7CF88]'>
      ...
      <TableCell className={`${isEdit && 'px-0 pb-0 pt-3 flex items-center h-full whitespace-nowrap'}`}>
        {isEdit ?
          <>
            <Input type="number" value={hour} className="border-none shadow-none text-2xl bg-white text-black h-full"
              onChange={(e) => setHour(parseInt(e.target.value))}
            />
            <div className="ml-1 h-full">시</div>
            <Input type="number" value={minute} className="border-none shadow-none text-2xl bg-white text-black h-full ml-3"
