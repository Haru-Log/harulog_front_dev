            ...
            {userList.map((user, index) => (
              <TableRow key={index}>
                ...
                <TableCell className="text-right w-[90px]">
                  {(user.nickname !== myName) && <Button className='bg-point rounded-lg font-bold shadow-sm hover:bg-point-hover active:bg-point-active'onClick={() => handleAddUser(user.nickname)}><UserRoundPlus color="#ffffff" />사용자 추가하기</Button>}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
