...
const MessageList = () => {
  ...
  return (
    <div>
        ...
        chatList.map((chats, index) => (
          <div
            className={`cursor-pointer hover:bg-gray-100 ${
              selectedChatroomInfo.roomId === chats.roomId ? 'bg-gray-200' : ''
            }`}
            // className={`cursor-pointer hover:bg-gray-100 `}
            key={index}
            ...
            <div className="text-sm py-3 whitespace-nowrap flex flex-row justify-between items-center">
              <div className="flex flex-row justify-start items-center">
                <div className="relative">
                  <MessageListImage
                    roomType={chats.roomType}
                    imgUrl={chats.users[0].profileImage}
                  />
                  <div>
                    {chats.unreadCount > 0 && (
                ...
                <span>
                  {truncateChatroomName(
                    getChatRoomName(chatList, chats.roomId),
                    20
                  )}
                </span>
              </div>
              <span className="text-xs text-gray-400 mx-3">
                {getTimeDifference(chats.updatedAt)}
              </span>
            </div>
            <Separator />
          </div>
      ...
      {chatList.length === 0 && (
        <span className="h-[1000px] flex items-center justify-center">
          참여중인 채팅방이 없습니다.
        </span>
      )}
    </div>
