...
import { addUsersExistChatRoom } from '@/api/chats/AddUsersExistChatRoom';
...
const ThisChatUserModal: React.FC<ThisChatUserModalProps> = ({
  usersInChat,
  onClose,
}) => {
  const {
  ...
  const handleAddUsers = async () => {
    ...
    await addUsersExistChatRoom(
      userWillAddedAfter,
      selectedChatroomInfo.roomId
    );
      ...
      <div className="w-auto h-auto z-50 bg-white p-8 rounded-3xl drop-shadow-xl">
        ...
        <div className="flex flex-row mt-5">
          {[...usersInChat, ...userWillAddedAfter].map((user, index) => (
            <div
              key={index}
              className="bg-point text-white px-3 py-2 rounded-2xl mr-2 flex flex-row items-center justify-between"
            >
              @{user}
            </div>
          ))}
        ...
        <div className="flex flex-row items-center justify-between mt-5">
          ...
          />
          <Button
            className="h-12 px-3 py-2 bg-point hover:bg-point-hover active:bg-point-active"
        ...
        <div className="mt-6 flex justify-between">
          <Button
            className="bg-main text-white font-semibold hover:bg-main-hover shadow-lg "
            onClick={onClose}
          >
            닫기
          </Button>
          <Button
            className="bg-point text-white font-semibold hover:bg-point-hover shadow-lg"
