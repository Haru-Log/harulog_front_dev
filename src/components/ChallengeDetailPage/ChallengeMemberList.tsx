...
import { joinChallenge } from '@/api/challenge/JoinChallenge';
    ...
    <div className="relative flex flex-col mt-10 mb-20 border-2 rounded-xl px-10 py-10">
      {!challenge.participate && (
        <div className="absolute inset-10 flex items-center justify-center z-50 blur-none">
          <Button
            className="bg-point text-white py-2 px-4 rounded-lg font-semibold shadow-lg hover:bg-point-hover active:bg-point-active"
            ...
            {challenge.challengeUserList?.map((member, index) => (
              <TableRow key={index}>
                ...
                <TableCell className="text-right w-[90px]">
                  <div
                    className={`text-white text-xs text-center font-bold w-fit h-fit px-3 py-3 rounded-lg shadow-sm ${
                      member.dailyAchievement === true
                        ? 'bg-green-300'
                        : 'bg-yellow-300'
                    } whitespace-nowrap`}
                  >
                ...
                <TableCell className="text-right w-[90px]">
                  <Button
                    className="bg-point rounded-lg font-bold shadow-sm hover:bg-point-hover active:bg-point-active"
                    onClick={() => navi(`/profile/${member.nickname}`)}
                  >
                    <UserSearch color="#ffffff" className="mr-2 h-5 w-5" />
                    프로필
                  </Button>
                </TableCell>
              </TableRow>
