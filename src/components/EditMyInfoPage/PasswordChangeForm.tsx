...
const PasswordChangeForm: React.FC = () => {
  ...
  const isPasswordValid: boolean = password.length === 0 || /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{10,}$/.test(password);
  ...
  return (
    ...
    >
      ...
      <div className="relative mb-4 mr-2">
        ...
        {!isPasswordValid && <p className="text-red-500 text-xs mt-2 ml-2">10자 이상 대, 소문자, 특수문자를 포함해야 합니다.</p>}
      ...
      {!isConfirmPasswordValid && <p className="text-red-500 text-xs mt-2 ml-2">비밀번호가 일치하지 않습니다.</p>}
